...
use super::{
    ...
    check_fragment_length, encode_string, finish, malformed, take, take_prefixed_string, too_large,
...
impl FragmentPatch {
    ...
    fn encode_into(&self, encoded: &mut Vec<u8>) -> Result<()> {
        match self {
            Self::SetText { target, value } => {
                encoded.push(1);
                encode_string::<u16>(encoded, target, "Fragment target identifier")?;
                encode_string::<u32>(encoded, value, "Fragment text value")?;
            }
            Self::SetAttribute {
                target,
                name,
                ...
                encoded.push(2);
                encode_string::<u16>(encoded, target, "Fragment target identifier")?;
                encode_string::<u16>(encoded, name, "Fragment attribute name")?;
                encode_string::<u32>(encoded, value, "Fragment attribute value")?;
            }
            Self::ReplaceChildren { target, text } => {
                encoded.push(3);
                encode_string::<u16>(encoded, target, "Fragment target identifier")?;
                encode_string::<u32>(encoded, text, "Fragment child text")?;
            }
        }
        Ok(())
    }
