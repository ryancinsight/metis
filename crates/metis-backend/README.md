...
The executable obtains a fresh key from the operating system and launches its
sibling `metis-frontend`. Build both binaries before running it. Windows process
tree containment bounds child lifetimes; it does not restrict file or network
permissions. The example policy is not clinical guidance. Audit storage does not
