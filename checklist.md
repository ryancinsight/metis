...
## root
- [Commands and events](backlog.md#METIS-COMMANDS-001): capability catalog, explicit unsupported-operation response, bounded subscription primitive, versioned remote event transport, typed plugin manifest registration with operation-count validation and backend-produced clinical.result display are implemented; remote plugin invocation remains.
