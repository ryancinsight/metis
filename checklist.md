...
## root
- [Commands and events](backlog.md#METIS-COMMANDS-001): capability catalog, explicit unsupported-operation response, bounded subscription primitive, versioned remote event transport, typed plugin manifest registration, scoped plugin invocation, backend-produced clinical.result display, and target capability discovery are implemented; browser-host delayed-response injection, native window, and OS permission providers remain.
