...
image example draws identity and clockwise quarter-turn placements from one
shared 3×2 source, then draws four visible line commands around the two views;
the regenerated SVG below is the inspected output of that combined path.

The focused `metis-ui-lang` suite passes 23/23 tests, including exact asymmetric
scaling, off-screen clipping, painter order, alpha-over-background and all five
pixel-grid orientation transforms. The bounded `examples/image.rs` workflow
...
not hand-authored. This closes the software raster image and discrete
pixel-grid-orientation subset of V06. Browser and native format decoding,
clinical orientation metadata, arbitrary affine transforms, font/media
lifecycles and GPU vector paths remain open.
