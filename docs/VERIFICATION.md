...
`628dee69887e73948f74336806313511b2aa4df9`; Atlas corrected the aggregate
source-distribution metadata extraction in merge
`f747b8b4de9fb39dfdbe3d3731f0020bc04a947d`, and the caller now pins that
revision for the confirming manual run.
...
semantics are shared with the inspected [software raster and vector presentation evidence](#software-raster-and-vector-presentation-evidence--2026-09-13).
...
[`native-host-frame.png`](manual/images/native-host-frame.png) and
...
<a id="browser-file-selection-evidence--2026-09-12"></a>
## Browser file-selection evidence — 2026-09-12

Metis revision `8c105e1` adds a user-activated HTML5 file chooser beside the
drop zone. The chooser delegates its `change` event to Moirai's
`WebEvent::selected_files` at revision `3caa6c24`; both sources use the same
bounded byte reader. No browser path, filesystem authority or DICOM logic
crosses this boundary. RITK consumer revision `52f5c52fbe3b8df18f275159ba7d79bd2a57c9c0`
owns classification, decoding, geometry and the three viewer canvases.

The live Codex in-app browser run opened the packaged RITK WebAssembly module
and used an automation file chooser to select the saved public MRI-DIR CT
series from `test_data/3_head_ct_mridir/DICOM`. These are 409 real Part 10
files, not generated bytes and not private patient data. The observed DOM
states were:

```text
input: type=file, multiple=true, accept=.dcm,application/dicom
Drop status: accepted 409 file(s); 00000001.dcm (528502 bytes), 00000002.dcm (528502 bytes), 00000003.dcm (528496 bytes); ...
Byte access: read 216156416 bytes from 409 file(s)
ritk-snap-axial: 512x512, axis=0, count=409, frame=presented, load=ready, slice=204
ritk-snap-coronal: 512x409, axis=1, count=512, frame=presented, load=ready, slice=256
ritk-snap-sagittal: 512x409, axis=2, count=512, frame=presented, load=ready, slice=256
```
...
cargo nextest run --offline -p metis-web --profile default — 38/38 passed
cargo clippy --offline -p metis-web --all-targets -- -D warnings — passed
cargo clippy --offline -p metis-web --target wasm32-unknown-unknown --all-targets -- -D warnings — passed
cargo build --offline -p metis-web --target wasm32-unknown-unknown --release — passed
cargo build --offline -p ritk-snap --lib --target wasm32-unknown-unknown --release — passed
```

The local stack overlay was active for the cross-repository package build, so
...
trace at source revision `b98f5881b3a528100e032e17892120f246bdf10a` returned
...
`32b487ea0ebd4e000c5b35b4ef70f75e04bf478027ada1754458d25590cc1283`). The
...
The fallback-chain increment (landed as `44e9130`, PR
[#238](https://github.com/ryancinsight/metis/pull/238)) adds three detached
samples for the requested `system-ui`, `sans-serif` and `monospace` families.
The Edge 154.0.4258.12 trace at device scale 1.25 is
`output/browser/runtime/edge-text-fallback-20260918.json` with SHA-256
`a4dab6a5e134ce0495115f9d600dfb9e05fc2fd54a0a6bc5e734f8dd091800d9`, bound to
revision `92064d65542c9a686e2c5d7d306af0ddb704b915`, and contains three
observations (`initial`, `remounted`, `remounted-cycle-2`). All samples report
`fonts_status: loaded` and `fonts_check: true`; their widths/heights are
`132.8/46.4`, `132.0/44.0` and `136.0/44.0` CSS pixels in requested order.
Because the trace is git-ignored host state, the artifact cited here is a
re-capture of the same command on this host: the original capture was taken
inside the ephemeral `feat/text-fallback-fonts` worktree and did not survive
that worktree's removal. The validator requires the exact chain, finite
positive bounds and detached probe cleanup. This records fallback-chain
admission and host metrics; browser APIs still do not identify the selected
system fallback face or establish native/cross-engine font parity.
...
## Software raster and vector presentation evidence — 2026-09-13
...
The same display list admits one-pixel line segments and bounded polyline
strokes. Widened integer clipping runs before Bresenham traversal, while the
width-aware path clips its miter envelope and classifies each visible pixel
once. Butt, square and round caps plus miter, bevel and round joins preserve
the existing source-over rule without repeated alpha blending. The image
example draws identity, clockwise quarter-turn and normalized affine shear
placements from one shared 3×2 source, then frames the first two with
round/round and square/bevel polylines; the regenerated SVG below is the
inspected output of that combined path.

The focused `metis-ui-lang` suite covers exact asymmetric scaling, off-screen
clipping, painter order, alpha-over-background, all five pixel-grid orientation
transforms, affine identity/reflection/scaling and invalid-matrix rejection,
plus the stroke command's style/point bounds. The bounded
`examples/image.rs` workflow
renders a 3×2 fixture into a 240×180 surface, asserts eighteen source-color
positions across the identity, rotated and affine placements plus the untouched
background, and writes `output/image-placement.svg` plus
`output/image-placement.bmp`. The SVG
generated by that run is committed as the [manual image artifact](manual/images/image-placement.png)
and was inspected through the BMP rendering; it is derived from the example,
not hand-authored. This closes the software raster image, discrete
pixel-grid-orientation, normalized affine placement and bounded stroke subset
of V06. Browser and native format decoding, clinical orientation metadata,
font/media lifecycles and GPU vector paths remain open.
...
with zero mounted controls and zero Rust-owned listeners before remounting at
...
The same hosted RITK workflow also selected a display-only maximum-intensity
...
provenance record reports mean peak private bytes of 416,867,669 (95%
half-width 30,202,762) and mean lifecycle duration of 2,123 ms (95% half-width
203 ms), with the requested logical size and observed display scale recorded
