...
example draws identity and clockwise quarter-turn placements from one shared
3×2 source, then frames them with round/round and square/bevel polylines; the
...
transforms and the stroke command's style/point bounds. The bounded
`examples/image.rs` workflow
renders a 3×2 fixture into a 240×180 surface, asserts twelve source-color
positions across the identity and rotated placements plus the untouched
...
pixel-grid-orientation and bounded stroke subset of V06. Browser and native
format decoding, clinical orientation metadata, arbitrary affine transforms,
