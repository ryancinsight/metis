...
The host conformance test records the complete format-neutral transition in
[`native-host-trace.json`](images/native-host-trace.json). At revision
`9934eeb5c05dedfdcd9c6da3088458b6ebaa07fe`, the trace contains an initial 2×2
blue frame, a `Resized` event that replaces it with a 3×2 green frame, a
repaint presentation with the exact row-major ARGB values, and a
`CloseRequested` batch that closes the live surface. The test also records
destruction after an application error. This is the host contract
demonstration; it does not parse DICOM data or claim a medical-viewer workflow.

The paired image is the production software framebuffer generated by the
existing presentation example from the same renderer used by the native
form. It excludes operating-system chrome so the pixels supplied to
`NativeSurface::present` can be inspected directly:
