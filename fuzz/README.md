...
This standalone harness exercises every public Metis wire decoder and the
bounded SVG admission parser with arbitrary bytes. It is outside the
application workspace, so `libfuzzer-sys` and its native runtime never enter
normal binaries or the locked release graph.
...
allocation growth beyond the decoder's 65,536-byte payload bound or the SVG
parser's 256 KiB input bound is a failure.
