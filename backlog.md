...
- Status: review; priority: P1; owner: Metis Python/platform; integrator: root; branch: current; dependencies: METIS-PYTHON-003, METIS-DESKTOP-001; risk: thread-affine native lifecycle
...
- Current increment (2026-09-11): `python scripts/python_binding.py` built the release `cp39-abi3` wheel and passed 22 value-semantic tests; the one free-threaded runtime probe was skipped because no free-threaded interpreter is installed. The committed `scripts/python_native_capture.py` tool then presented a checkerboard through a visible HWND and recorded [`python-native-window.bmp`](docs/manual/images/python-native-window.bmp) at source revision `c27326419684eefba59901dea52fb3f7077e5457`. Rust formatting and warning-denied Clippy passed for `metis-python`; two-window lifecycle coverage is now in the wheel suite, while trusted IME and non-Windows provider evidence remain open under V05.
- Current increment (2026-09-11): native provider submission, finite waits, close and reopen release the Python interpreter lock through PyO3 detached regions; the extracted-wheel suite and native capture contract remain unchanged. Trusted IME and non-Windows provider evidence remain open under V05.
- Current increment (2026-09-12): the bounded capture utility decodes every PNG scanline filter and rejects malformed checksums before presenting the exact RGBA pixels from the public RITK 409-file CT/MIP frame. The visible 1296 × 839 window shows four non-black RITK panels; its 1280 × 800 client crop matches the source at `(8, 31)` with zero pixel differences. Source and output digests are recorded in [the capture manifest](docs/manual/images/python-native-captures.json). Moirai provider rendering is merged in [PR #329](https://github.com/ryancinsight/Moirai/pull/329). Private patient frames remain local; DICOM ownership remains RITK.
