...
- Historical resource increment (2026-09-19; superseded by the geometry lock): `scripts/resource.py` ran three bounded lifecycle samples against the prior standalone-lock RITK/Métis executable and 94-file public MRI-DIR study. Peak private bytes averaged 815,602,347 ± 195,407; peak working set averaged 385,309,355 ± 6,584,234; lifecycle duration averaged 2,041 ± 24 ms; all exits and capture hashes passed. The record binds the prior Metis, RITK and Moirai revisions and remains a fixture measurement rather than a framework ranking. A fresh resource sample for the geometry-aware executable is still required before those numbers can be treated as current. Matched GPUI/Tauri/egui, WASM allocator, compositor and ranking residuals remain open.
