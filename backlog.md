...
- Status: in-progress; priority: P1; owner: Metis protocol/browser; integrator: root; last-update: 2026-09-16; branch: `docs/metis-fragment-close`; regions: `backlog.md`; dependencies: METIS-BROWSER-001, METIS-COMMANDS-001, METIS-AUTHORITY-001; risk: remote markup and stale lifecycle
- lease: root backlog.md (2026-09-16T17:00:00-04:00)
...
- Completed increment: `FragmentAction`, `FragmentPatchSet`, the scoped `ui` plugin and the WASM target policy use the existing authenticated plugin envelope; all mutations are bounded text/attribute operations with generation checks and atomic preflight. The HTTP browser probe now streams through a 16 KiB body bound, uses exact attribute policy and leases every request to the current mount, so reset clears old fragment state before stale completion can update the page. No DICOM knowledge or dependency enters Metis; RITK remains the format and viewer owner.
...
- Protocol increment (2026-09-13): the HTTP page rejects an unallowlisted target without changing its prior text, and `scripts/browser_fragment.py` records authenticated success, malformed/unauthorized/target/stale probes, reset and remounted success in a bounded WebDriver trace. Protocol-shaped tests pass; the live visual captures below bind the configured endpoints.
- Live visual increment (2026-09-14, Metis `ed3806811f23271310cb04078dff55aba5c90944`): the Codex in-app Chromium host ran the current loopback `metis-app.exe` service and captured authenticated success, **Reset mount** generation `2` with empty stale state, and remounted authenticated success. The [provenance record](docs/manual/images/metis-http-fragment-live.json) binds the service, executable digest, viewport dimensions and image hashes; the [manual](docs/manual/browser.md#current-live-fragment-capture--2026-09-14) links the three inspected JPEGs. The probe has no DICOM or patient data; RITK remains the viewer owner.
- Edge WebDriver increment (2026-09-14, Metis `77b1c278d610562b04d138aea2d37b828ff097a5`): the bundled Edge driver ran the authenticated fragment, rejection, reset and remount scenario against Edge `154.0.4258.12`, captured three semantic PNG states at device scale `1.25`, and closed with no pending request. The [provenance](docs/manual/images/metis-http-fragment-webdriver-edge.json) and [manual](docs/manual/browser.md#live-edge-webdriver-fragment-capture--2026-09-14) bind the trace and screenshots; this is one Chromium-family driver observation with no DICOM bytes.
- Cross-engine workflow increment (2026-09-16, Metis `07fe83175b7952177199340ad078fd0cfd90f012`): the scheduled/manual `browser-runtime` matrix builds the real `metis-app` HTTP service through the shared OS-port static-server helper, serves `http-health.html`, waits for an Origin-authorized `/health` response, and runs the authenticated fragment trace for Chromium, Firefox and WebKit. Each engine uploads its bounded semantic trace and three PNG states.
- Hosted cross-engine evidence (2026-09-16, run [35143857171](https://github.com/ryancinsight/metis/actions/runs/35143857171), trace revision `d6e52068708b72becf2a4173d875a4b367789c6e`): Windows, fuzz, lockfile, workflow, ADR, asset-build and all three browser jobs passed. Every engine recorded health `200`, fragment `200 (1 patch)`, malformed `400`, unauthorized `401`, target rejection, stale unchanged, reset/remount generation `2`, idle request state and `session_closed: true`; per-engine artifacts and screenshot hashes are in the [manual](docs/manual/browser.md#hosted-fragment-matrix-evidence--2026-09-16) and [verification record](docs/VERIFICATION.md#browser-fragment-hosted-cross-engine-evidence--2026-09-16).
...
- Residuals: provider-private listener/allocation counts remain open; Chromium is the only engine with `performance.memory`. TLS, accessibility/IME, OS permissions and physical input remain separate host gaps. The exact full gate and locked WASM library build still govern protocol changes. DICOM parsing, decoding and viewer semantics remain RITK-owned.
...
- Current increment: `metis-platform::native::WebViewSurface` consumes the merged provider through a safe adapter. It sizes the controller, maps validated visibility, forwards combined window/WebView events and preserves bounded navigation and JSON messaging. Metis uses git-plus-version Moirai requirements; Cargo.lock records provider revision `c110452ec8a8057a98deab330f9047b1c7efd522`. Configuration and ignored installed-runtime adapter tests pass on WebView2 `152.0.4191.66`, covering packaged navigation, the ready bridge message, external-navigation denial and close. The visible bundle journey is captured in [`docs/manual/images/native-captures.json`](docs/manual/images/native-captures.json).
...
- Decision: [ADR 0015](docs/adr/0015-native-window-provider.md) (accepted); committed visual captures cover the current native/WebView2 initial and submit journeys plus the native framebuffer resize. Installed IME, WebView2 composition, OS permission enforcement, accessibility and macOS/Linux providers remain open follow-on slices.
