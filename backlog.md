...
- Status: in-progress; priority: P1; integrator: root; last-update: 2026-09-18; dependencies: METIS-BROWSER-CAPTURE-001.
...
- Evidence: dependency-free checks pass 283/283 with one intentional skip; the pinned full Metis verifier passes all 26 stages at revision `602d6e2196655bb2a1bfee1763869c381e338aa3`, with the expected `capture-failure` probe returning `PermissionDenied` and zero visual pixel or semantic differences. Hosted manual run [35368255350](https://github.com/ryancinsight/metis/actions/runs/35368255350) passes Chromium, Firefox, WebKit and the cross-engine comparator; artifact `metis-browser-text-cross-engine-35368255350` records five observations per engine and one semantic contract.
...
- Completed increment (2026-09-17): commit `be766e8` adds the browser `--asset-probe`, which decodes the local SVG and PNG marks with `HTMLImageElement.decode()`, records same-origin intrinsic dimensions, and removes the probe container before each lifecycle remount.
- Evidence: the dependency-free suite ran 220 tests with one expected skip; `python -S scripts/verify.py` passed all stages at branch revision `c752ae1` with 170 resolved packages and the deliberate `capture-failure` stage exiting 1. The scheduled browser matrix invokes the probe for Chromium, Firefox and WebKit.
