...
- Completed increment (2026-09-12): `scripts/resource.py` measures one real command and its visible process tree with bounded Windows and `/proc` samplers, redacted command fingerprinting, timeout termination and initial/final/peak/growth summaries. Five focused tests and the full Metis gate pass. A local saved Patient_01 DICOM run exited 0 and produced a real capture; the report remains local because its input is private.
