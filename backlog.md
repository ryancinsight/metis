...
- Residuals: configured cross-engine WebDriver success/rejection/stale-generation captures and provider-private listener/allocation counts remain open. The current in-app Chromium capture is one-engine evidence; TLS, accessibility/IME, OS permissions and physical input remain separate host gaps. The exact full gate and locked WASM library build still govern protocol changes.
