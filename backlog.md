...
- Status: in-progress; priority: P1; owner: Metis frontend/host; integrator: root; last-update: 2026-09-16; regions: scripts/browser_drop.py, scripts/browser_protocol.py, scripts/browser_canvas_capture.py, scripts/tests/test_browser_drop.py, scripts/tests/test_browser_runtime.py, docs/manual/browser.md, docs/VERIFICATION.md, backlog.md; dependencies: METIS-STATE-001, METIS-ASYNC-001, METIS-AUTHORITY-001; risk: browser/native trust boundary
...
- Canvas capture increment (2026-09-16): `browser_drop.py` retains exact RGBA capture by default and accepts explicit `--canvas-capture screenshot --canvas-context <name>` for non-2D consumers. The mode validates intrinsic dimensions and context availability, records real element PNG digests, and checks rejection stability without interpreting consumer pixels. ADR [0037](docs/adr/0037-browser-canvas-capture.md) records the boundary; RITK owns the WebGPU/DICOM visual oracle.
- Canvas capture delivery (2026-09-16): Metis PR [#190](https://github.com/ryancinsight/metis/pull/190) merged at `29812898870042665010c6534dce111ea1966925` after hosted run [35121684038](https://github.com/ryancinsight/metis/actions/runs/35121684038) passed the Windows, workflow, lockfile and ADR gates. Browser visual evidence remains consumer-owned; the RITK saved-study WebGPU workflow is the active presentation check.
- Consumer harness boundary (2026-09-16): removed the former split slice-control runner and its RITK-specific modules from Metis. `browser_drop.run` now accepts an explicit single-lifecycle consumer-capture callback, records only its bounded JSON mapping, and uses the generic window-rectangle API. RITK owns the moved harness and its DICOM assertions under [RITK-BROWSER-GALLERY-HARNESS-001](../ritk/backlog.md#RITK-BROWSER-GALLERY-HARNESS-001).
...
- Completed increment (2026-09-11): the paired Edge run loaded the public 409-file CT series through the bounded file-drop path, then moved all three RITK canvases with trusted wheel actions. Metis captured six semantic snapshots and eight PNGs; RITK `ritk-snap --validate-browser-trace` passed against Metis `1fd44680d1a15a12efda2e2cf0f2ed560da92db6` and RITK `0bad9eb2c77b91e57fff3e9dc472a42201e061aa`. The trace and PNGs are in [`docs/manual/images`](docs/manual/images/browser-gallery-canvas.json).
...
- Completed increment (2026-09-11, local branch `feat/metis-browser-canvas-001`): `metis-web` exposes a format-neutral borrowed `CanvasFrame` seam and a WASM `CanvasSurface` over Moirai's bounded HTML5 canvas provider. The standalone lock now resolves Moirai to merged PR #321 (`ccdc878d`). RITK's `PresentationFrame` is the consumer adapter; no DICOM parser, metadata, geometry or viewer state enters Métis. Native tests and strict native/WASM clippy pass; the visual baseline provenance was refreshed with unchanged capture pixels. RITK PR #284 records a local synthetic browser canvas smoke through this seam, and RITK PR #290 now routes bounded pointer and wheel actions through its viewer reducer; trusted physical input, cross-engine driver evidence, GPU paths and full-window capture remain open.
- Physical-input transport increment (2026-09-11, Metis PR [#68](https://github.com/ryancinsight/metis/pull/68), merge `df41e55`): the dependency-free W3C client now validates and dispatches bounded pointer-drag and wheel action sources, releases active sources explicitly, and rejects malformed or over-budget sequences before transport. The seam carries element-local browser values only; RITK remains responsible for DICOM and viewer semantics. Python protocol tests pass; configured Chromium/Firefox/WebKit captures remain required for the owning consumer workflow.
- Consumer-canvas trace increment (2026-09-11, branch `feat/browser-canvas-trace`): `browser_runtime.py --scenario canvas` now captures full-window and per-canvas PNGs, records bounded intrinsic/CSS dimensions, dispatches trusted pointer-drag and wheel actions to a finite canvas-ID list, releases all input sources, and carries separate Metis/consumer revisions. The deterministic Python suite passes 15/15; the runner remains format-neutral and RITK supplies DICOM and viewer assertions.
- Completed increment (2026-09-11, Metis PR [#73](https://github.com/ryancinsight/metis/pull/73), commit `fb4ad93`): the canvas trace accepts a bounded repeated `--canvas-attribute data-*` allowlist and records requested values opaquely, including explicit null for absent attributes. Metis does not interpret DICOM or viewer meaning; RITK supplies and interprets its `data-ritk-*` contract through the manual workflow.
- Completed increment (2026-09-11, Edge WebDriver): `browser_drop.py` ran against the current gallery with the public 409-file MRI-DIR CT study. The trace records Edge 153.0.4234.19, Metis `0f7093e6701af4ab4a3779925206d7a31f964319`, RITK `0bad9eb2c77b91e57fff3e9dc472a42201e061aa`, trusted file-backed drag events, per-file hashes, exact three-canvas RGBA oracles, three overflow rejections, window/element PNGs and `session_closed: true`; the refreshed provenance is `docs/manual/images/browser-gallery.json`.
- Completed increment (2026-09-11, Metis `1321bd434500744e4d80fb906d10d9aa74590003`, RITK `0bad9eb2c77b91e57fff3e9dc472a42201e061aa`): pointer evidence now requires every phase (`pointerdown`, `pointermove`, `pointerup`) and rejects incomplete or untrusted event mutants. The real Edge trace and RITK validation remain bound to the accepted public study in [`docs/manual/images`](docs/manual/images/browser-gallery-canvas.json).
- Completed increment (2026-09-12, Metis `8c105e1`, Moirai `3caa6c24`, RITK consumer `52f5c52fbe3b8df18f275159ba7d79bd2a57c9c0`): the user-activated `#file-input` chooser now feeds Moirai's bounded browser file handles through the same 512-file, 64 MiB/file and 256 MiB/batch reader as drag/drop. The in-app Chromium runs selected the 409 real public CT files (216,156,416 bytes) and the 94-file MRI study (49,807,236 bytes), presenting ready non-black axial, coronal and sagittal canvases; [the exact DOM and build evidence](docs/VERIFICATION.md#browser-file-selection-evidence--2026-09-12) records both traces, attributes and revisions.
- Completed increment (2026-09-13, Metis `1ea339107780ba6b1f5f4808bd9c7409418943c1`, RITK `1547d2af4cbb58d7c0630e422b2b9903902c9811`, evidence merged in RITK PR #336): Microsoft Edge 154.0.4258.12 accepted the saved public 94-file MRI-DIR T2 study through `browser_drop.py`, read 49,807,236 bytes, matched all three RITK RGBA oracles, rejected count/file/batch overflow before reading, and closed cleanly. The [RITK gallery capture and provenance](https://github.com/ryancinsight/ritk/blob/main/docs/manual/images/dicom-metis-real-browser-mri-edge.json) bind the actual images and trusted transfer; Edge remains one Chromium-family engine and file-backed WebDriver input is not physical file-manager drag.
- Completed increment (2026-09-13, commits `b963144`, `a2ec228`, `e985319`): the mounted browser root now publishes the Rust-owned listener count and monotonic lifecycle generation as bounded attributes and visible status. Stop records zero retained listener guards; remount records a new generation and a positive listener count. The W3C workbench runner asserts both values alongside the stale-response and pending-request checks, while provider-private resource counts remain outside the WebDriver contract. The focused Python suite and neutral full gate pass.
- Completed increment (2026-09-13, commits `3ad07fd`, `b154026`): the W3C runner accepts `MicrosoftEdge` as a validated Chromium-family browser name for both workbench and canvas scenarios. A live Edge 154.0.4258.12 run passed the disconnected workbench input, disabled-submit, stop/remount and JavaScript heap observations; the sanitized trace and five inspected screenshots are in [`docs/manual/images`](docs/manual/images/metis-browser-edge-workbench.json). The stop assertion now accepts the visible lifecycle status while requiring the exact stop message and zero listener handles.
- Completed increment (2026-09-13, Metis PR #115): the workbench runner repeats the stop/remount contract for a bounded `--lifecycle-cycles` value and records each generation, listener count and mounted-control count. This extends the browser lifecycle assertion without moving DICOM or provider-private resource ownership into Metis; a live multi-cycle engine trace remains a V12 requirement.
- Completed increment (2026-09-13, Metis `012ed7175268c283cb4b8cbd617f1a138d955532`): a live Microsoft Edge 154.0.4258.12 run with msedgedriver 153.0.4234.19 completed four bounded stop/remount cycles. Stopped generations 3, 5, 7 and 9 reported zero controls/listeners; remounted generations 4, 6, 8 and 10 reported 27 controls/31 listeners. The sanitized trace and five inspected PNGs are in [`docs/manual/images`](docs/manual/images/metis-browser-edge-lifecycle.json); provider-private resources and non-Chromium engines remain open.
- Completed increment (2026-09-13, hosted run `34759186816`, Metis `67689e5001f65d22ac388397442eb99df9bfe0ef`): the locked asset build fed Chromium 152.0.7977.82, Firefox 155.0 and Safari 26.6.2. All three four-cycle disconnected workbench traces passed two input changes, zero stopped listeners, 31 remounted listeners, zero pending requests and clean session teardown; each uploaded five PNG captures. The [run artifacts](https://github.com/ryancinsight/metis/actions/runs/34759186816) are bound to the exact revision.
- Completed increment (2026-09-13, commits `55e5665` and `abd6578`): the canvas trace derives and records element-local pointer and wheel offsets from each canvas's CSS dimensions. The live Edge replay of the public 94-file MRI-DIR T2 study accepted 49,807,236 bytes, moved axial 47→46 and coronal/sagittal 256→255 through trusted actions, passed RITK validation and released all 12 diagnostic listeners and input sources. The prior fixed wheel point was outside the short coronal and sagittal surfaces; the adaptive clamp keeps every action in bounds. See [browser canvas input-offset evidence](docs/VERIFICATION.md#browser-canvas-offset-evidence--2026-09-13).
- Completed increment (2026-09-14, branch `feat/metis-browser-file-chooser-matrix`): `browser_drop.py` now resolves the Chromium/Firefox/WebKit matrix and drives the real `#file-input` through bounded W3C absolute-path selection. The chooser records trusted `change` evidence, per-file hashes, exact consumer RGBA values and the same count/file/batch rejection probes; the existing Chromium CDP drag and manual observation paths remain explicit. The 140-test dependency-free suite passes.
- Completed increment (2026-09-14, hosted RITK workflow [34895454734](https://github.com/ryancinsight/ritk/actions/runs/34895454734)): Chromium 152 and Firefox 155 accepted the real 94-file MRI-DIR T2 study, matched all file and three-canvas RGBA oracles, rejected the bounded overflow probes, validated focused trusted `ArrowDown` keyboard traces on all three canvases, and closed cleanly. Safari 26.6.2 accepted the chooser paths but rejected the first bounded browser read before canvas presentation; the exact galleries, traces and provenance are recorded in [RITK's manual evidence](https://github.com/ryancinsight/ritk/tree/main/docs/manual/images). Metis `02d4047`, RITK `92f4dc5` and Moirai `c110452` are the source revisions.
- Hosted fragment increment (2026-09-16, Metis run [35143857171](https://github.com/ryancinsight/metis/actions/runs/35143857171)): the generic HTTP service trace passes on Chromium 152, Firefox 155 and WebKit/Safari 26.6.2 with health/fragment success, bounded rejection probes, generation reset/remount and clean teardown. This closes the format-neutral fragment matrix; it carries no DICOM bytes. The saved-study WebKit read remains the separate provider residual.
- Delivery update (2026-09-14): Metis PR [#140](https://github.com/ryancinsight/metis/pull/140) merged at `85c6c8b`; the chooser-matrix manual and verification records are now on `main`. The runner implementation remains at `0a5a69d` and is format-neutral; Safari's first bounded file read remains a provider/consumer residual recorded above.
- Current replay reconciliation (2026-09-17): RITK PR [#464](https://github.com/ryancinsight/ritk/pull/464), merge `132251fa57046241e55cb9189126d6ae7fb98eb9`, replays the saved 94-file MRI-DIR study through the format-neutral Métis surface using Metis `8e566af9a37dc0382e8e919c593d3838f5b08186` and Moirai `a2f21496d1d09b2abe6523e3c8cdbf751dcd560a`. The replay reads 49,807,236 bytes, reproduces the revision-bound 1280 × 800 MRI frame, and rejects the invalid-study probe; the [RITK provenance](../ritk/docs/manual/images/dicom-metis-real-mri.json) carries the executable and image hashes. Chromium and Firefox browser galleries remain the real cross-engine evidence; Safari's selected-file read and hosted Chromium WebGPU adapter remain explicit host residuals.
- Dependency-lock increment (2026-09-14): Metis Cargo.lock now resolves all eleven Moirai packages to merged PR #342 (`27d9b06190e8e3db930ef9745187e0ea48cadfce`), including the bounded object-URL response reader. Metis PR [#148](https://github.com/ryancinsight/metis/pull/148) merged at `c093f279e986ce81b2f8db292510fbd7f1297f9b`; the standalone lock check and focused browser-host gates run against the new provider; RITK hosted saved-study replay `34905504112` is the downstream acceptance run.
- Dependency-lock delivery (2026-09-14): Metis PR [#150](https://github.com/ryancinsight/metis/pull/150) merged at `befc5333ff4869e632eb38b14997ce865e4dfbe9`; its standalone lock records Moirai PR #342 at `27d9b06190e8e3db930ef9745187e0ea48cadfce`. RITK PR [#388](https://github.com/ryancinsight/ritk/pull/388) carries the six Metis package sources and workflow default at this exact revision; hosted saved-study replay run [34911806660](https://github.com/ryancinsight/ritk/actions/runs/34911806660) is the pending downstream acceptance run.
- Browser policy correction (2026-09-14): Metis `connect-src` now admits only the `blob:` scheme needed by Moirai's bounded object-URL response stream; the generated gallery policy, host assertion, manual and ADR are synchronized. A real Chromium probe with the gallery policy reproduced the prior `Failed to fetch` error without `blob:` and succeeded after the source was admitted; the RITK cross-engine acceptance rerun remains required.
- Completed provider-read increment (2026-09-15, Metis PR [#153](https://github.com/ryancinsight/metis/pull/153), merge `fa7793be9b738859696ad566e012d69a763e55ca`; Moirai PR [#344](https://github.com/ryancinsight/Moirai/pull/344), merge `3ccfb1b76f386321aa24a86e65fe81efdbdfad84`): the consumer sizes its first bounded request to the file when it is at most 1 MiB, selecting Moirai's `File.arrayBuffer()` path for Safari while retaining object-URL continuation reads for larger or positioned requests. Local WASM, Clippy, nextest 40/40 and Python browser tests pass; hosted RITK replay remains the acceptance oracle.
- Dependency-lock delivery (2026-09-15): all eleven Moirai packages now resolve to merged PR #344 at `3ccfb1b76f386321aa24a86e65fe81efdbdfad84`; the standalone lock and focused browser-host gates pass. The downstream RITK lock and hosted saved-study replay remain the acceptance step.
- Dependency-lock increment (2026-09-15, branch `build/metis-moirai-size-lock`): all eleven Moirai packages now resolve to merged PR [#353](https://github.com/ryancinsight/Moirai/pull/353), main `2451a3155c44dcf76d5577e4eb8c08badde51a0a`, which validates browser file sizes at the provider boundary. The standalone lock and focused Metis browser-host gates are the acceptance checks; the downstream RITK lock and hosted saved-study replay remain required, and Safari/WebKit authorization remains the recorded external residual.
- Hosted evidence delivery (2026-09-14): RITK PR [#384](https://github.com/ryancinsight/ritk/pull/384), merge `df388a2aa`, records the Chromium/Firefox keyboard validation, actual galleries, per-engine artifact links and Safari's bounded-read residual in the RITK manual and provenance.
- Hosted evidence update (2026-09-15): RITK workflow [34973438029](https://github.com/ryancinsight/ritk/actions/runs/34973438029) binds Metis `b374ca937ce6ddbcbce2fb55a0dd074a241ea956`, RITK `67ed6db952414d7b611e57a8100deead0eba7e88` and Moirai `2451a3155c44dcf76d5577e4eb8c08badde51a0a`. Chromium 152 and Firefox 155 pass the real 94-file MRI-DIR T2 chooser, exact file/RGBA oracles, bounded rejections, trusted `=`/`-` cine-rate repeat handling and clean teardown; Safari 26.6.2 accepts the chooser but fails the first bounded read before DICOM or keyboard presentation. RITK owns the actual galleries and provenance; Metis remains format-neutral.
- Page-query increment (2026-09-16): the generic file-backed runner accepts up to eight validated `--page-query KEY=VALUE` parameters, appends them to the same-origin gallery URL and waits on the shared `Ready` status without interpreting consumer mode. Duplicate keys, delimiters and over-budget values fail before a session starts. This gives RITK an explicit `renderer=webgpu` route without a second transport runner or DICOM policy in Metis; the consumer still owns WebGPU capability and visual evidence.
- Lease discharged: page-query runner, focused tests and manual/verification documentation are delivered on this branch; RITK consumer integration remains the next cross-repository increment.
- Residuals: WebKit acceptance of the direct bounded file read, physical file-manager input, provider-private listener/resource counts, TLS, accessibility/IME, post-drop allocation and native desktop evidence remain open; DICOM parsing and viewer semantics stay with RITK under [RITK-SNAP-DICOM-SUBSTRATE-001](../ritk/backlog.md#RITK-SNAP-DICOM-SUBSTRATE-001).
...
- Current increment: `metis-platform::native::WebViewSurface` consumes the merged provider through a safe adapter. It sizes the controller, maps validated visibility, forwards combined window/WebView events and preserves bounded navigation and JSON messaging. Metis uses git-plus-version Moirai requirements; Cargo.lock records provider revision `c7b49a7623aed533f377aec77f656a16d2b9d68b`. Configuration and ignored installed-runtime adapter tests pass on WebView2 `152.0.4191.66`, covering packaged navigation, the ready bridge message, external-navigation denial and close. The visible bundle journey is captured in [`docs/manual/images/native-captures.json`](docs/manual/images/native-captures.json).
...
- Status: in-progress; priority: P1; owner: Metis UI/host; integrator: root; last-update: 2026-09-15; regions: `crates/metis-python/src/native/events.rs`, `crates/metis-python/tests`, `docs/manual/python.md`, `docs/adr/0026-python-native-host.md`, `backlog.md`; dependencies: METIS-BROWSER-001, MOI-WINDOW-KEY-MODIFIERS-2026-09-15; risk: native shortcut modifier state loss; ADR: 0014, 0027
...
- Residuals: device-loss/recreate handling, browser vector parity, a measured GPU profile and real browser GPU evidence remain open.
