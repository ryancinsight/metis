...
- Follow-on provider: Moirai branch `arch/webview2-provider` at `9e045f73` adds the thread-affine WebView2 host, packaged `file:///` navigation policy, bounded WebMessageReceived bridge and callback teardown. The [Moirai provider item](../moirai/docs/backlog.md#MOI-WINDOW-WEBVIEW2-2026-09-09) remains in progress until an installed-runtime capture proves the Metis bundle integration; no registry or signing key is part of that provider.
- Current increment: `metis-platform::native::WebViewSurface` now consumes that provider through a safe adapter. It sizes the controller, maps validated visibility, forwards combined window/WebView events and preserves bounded navigation and JSON messaging. All direct Moirai dependencies are pinned to the same provider commit for co-evolution; the pin is removed after the provider merges to Moirai main. Adapter configuration tests pass; the installed-runtime smoke and visible bundle capture remain open.
- Current increment: `metis-app --metis-webview` now runs the same one-executable supervised process workflow through a visible WebView2 form. A bounded temporary HTML/CSS package uses a no-network CSP and typed JSON bridge; submit messages traverse the unprivileged frontend's private pipe and return the capability-authorized, audited result to the page. Installed-runtime, visible capture, permission and accessibility evidence remain open.
...
- Status: blocked; priority: P3; owner: Metis delivery; integrator: root; last-update: 2026-09-09; branch: `feat/process-foundation`; dependencies: METIS-DISTRIBUTION-001, METIS-CONFORMANCE-001; blocker: crates.io trusted-publisher registration and explicit release authority are external; re-open: registrations and release authority are available without adding repository secrets
...
- Current increment: the crates.io caller is pinned to Atlas's reusable OIDC workflow and separates validation-only dispatch from release-only publication; its contract tests and committed full gate pass. Local crates.io publish validation is blocked by index network access, while registry registration and release authority remain open. The PyO3 package and PyPI caller are tracked by [METIS-PYTHON-001](#METIS-PYTHON-001); PyPI trusted-publisher registration remains an external release action.
- Completed increment (2026-09-08): the public manual records the exact crates.io and PyPI trusted-publisher fields, the `crates-io`/`pypi` environment boundary and the prohibition on registry secrets, passwords and signing keys. Live GitHub API inspection confirms the public repository has zero Actions secrets, variables, environments or rulesets; no private-key path is configured. Registry publisher registration and named environment creation remain release-owner actions.
- Recheck (2026-09-09): the Metis remote is HTTPS, local Git has no signing or SSH-key configuration, and the checked-in callers still contain no registry secret path. The current `gh` credential is invalid and the visible browser is signed out, so environment creation and trusted-publisher registration remain the only external steps; no key or token is requested from the developer.
