...
- Status: in-progress; priority: P1; owner: Metis frontend/host; integrator: root; last-update: 2026-09-14; branch: `feat/metis-browser-file-chooser-matrix`; regions: `scripts/browser_drop.py`, `scripts/browser_protocol.py`, `scripts/tests/test_browser_drop.py`, `scripts/tests/test_browser_runtime.py`, `docs/manual/browser.md`, `docs/VERIFICATION.md`, `backlog.md`; dependencies: METIS-STATE-001, METIS-ASYNC-001, METIS-AUTHORITY-001; risk: browser/native trust boundary
...
- Completed increment (2026-09-14, branch `feat/metis-browser-file-chooser-matrix`): `browser_drop.py` now resolves the Chromium/Firefox/WebKit matrix and drives the real `#file-input` through bounded W3C absolute-path selection. The chooser records trusted `change` evidence, per-file hashes, exact consumer RGBA values and the same count/file/batch rejection probes; the existing Chromium CDP drag and manual observation paths remain explicit. The 140-test dependency-free suite passes. Live Firefox/WebKit saved-study captures remain open until their configured drivers are available.
- Residuals: physical file-manager input, provider-private listener/resource counts, TLS, accessibility/IME, post-drop allocation and native desktop evidence remain open; DICOM parsing and viewer semantics stay with RITK under [RITK-SNAP-DICOM-SUBSTRATE-001](../ritk/backlog.md#RITK-SNAP-DICOM-SUBSTRATE-001).
