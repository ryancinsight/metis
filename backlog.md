...
- Replay [35167488704](https://github.com/ryancinsight/metis/actions/runs/35167488704) fails all browser jobs before fragments; Safari reports a JavaScript parse error in the preceding accessibility probe. Removing its surplus closing parenthesis passes Node syntax validation and 98 focused Python tests. The request-state change passes the configured local gate (171 packages); corrected cross-engine replay remains required.
- Replay [35167864608](https://github.com/ryancinsight/metis/actions/runs/35167864608) at `13e7e01` passes Chromium; Firefox's real service exits with `FrameTruncated` before the fragment completes. The listener currently propagates a peer-local read failure as a terminal service failure. [ADR 0025](docs/adr/0025-axum-server-boundary.md) mandates bounded connection isolation and typed host reporting; its serving-API observer argument requires caller migration.
- Production-fix evidence: configured `python -S scripts/verify.py` passes (171 packages; 330 debug and 330 release tests, one existing skip each), including the real peer-failure regression. `cargo semver-checks 0.50.0` detects only the observer argument additions (222 checks pass, one breaking category fails); the API migration is intentional and documented, with no release/version bump. Cross-engine rerun remains required.
- Replay [35168974217](https://github.com/ryancinsight/metis/actions/runs/35168974217) at `a9e7172` passes Chromium, Firefox and the configured hosted gate; WebKit fails static port publication before service launch, with an empty log. A real-HTTP regression reproduces the static server's unnecessary reverse-DNS dependency and passes after its removal; this does not establish the hosted stall's cause. Startup now emits a stack after ten seconds within the unchanged twenty-second host bound. The 99 focused tests and configured local gate pass; WebKit replay remains required.
