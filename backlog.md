...
- Completed increment (2026-09-13): the mounted browser root now publishes the Rust-owned listener count and monotonic lifecycle generation as bounded attributes and visible status. Stop records zero retained listener guards; remount records a new generation and a positive listener count. The W3C workbench runner asserts both values alongside the stale-response and pending-request checks, while provider-private resource counts remain outside the WebDriver contract. The focused Python suite and neutral full gate pass.
