...
- Status: in-progress; priority: P1; owner: Metis protocol/browser; integrator: root; last-update: 2026-09-16; branch: `fix/metis-fragment-static-server`; regions: `.github/workflows/ci.yml`, `scripts/browser_static_server.py`, `scripts/browser_fragment.py`, `scripts/browser_runtime.py`, `scripts/tests/test_browser_fragment.py`, `scripts/tests/test_browser_runtime.py`, `docs/manual/browser.md`, `docs/VERIFICATION.md`, `backlog.md`; dependencies: METIS-BROWSER-001, METIS-COMMANDS-001, METIS-AUTHORITY-001; risk: remote markup and stale lifecycle
...
- Cross-engine workflow increment (2026-09-16, branch `feat/metis-fragment-cross-engine`): the scheduled/manual `browser-runtime` matrix now builds the real `metis-app` HTTP service, serves `http-health.html`, waits for an Origin-authorized `/health` response, and runs the authenticated fragment trace for Chromium, Firefox and WebKit. Each engine uploads its bounded semantic trace and three PNG states; the next hosted run is the acceptance evidence for the remaining engine residuals.
...
- Residuals: Firefox/WebKit WebDriver captures and provider-private listener/allocation counts remain open. The current Edge and in-app Chromium captures are Chromium-family evidence; TLS, accessibility/IME, OS permissions and physical input remain separate host gaps. The exact full gate and locked WASM library build still govern protocol changes.
