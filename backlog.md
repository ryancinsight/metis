...
- Status: in-progress; priority: P1; owner: Metis delivery; integrator: root; branch: `fix/metis-python-platform-validation`; last-update: 2026-09-20; dependencies: METIS-PYTHON-003, ATLAS-PUBLISH-001; risk: hosted CPython ABI coverage
...
- Current increment (2026-09-20): the release caller advances to Atlas merge `628dee69887e73948f74336806313511b2aa4df9`, which owns the shared `cp314t`/`cp315t` and Python 3.15 `abi3t` matrix plus compressed macOS universal2 tag validation; the default remains CPython 3.9 `abi3`. A manual `workflow_dispatch` selects Atlas validation mode and produces a retained verification artifact without release attachment or PyPI publication; the caller and package remain tokenless OIDC, and musllinux stays outside `abi3t` until a compatible 3.15t image exists. Hosted artifact and value-test evidence remain pending under the manual path.
