...
- Evidence: hosted RITK run [34973438029](https://github.com/ryancinsight/ritk/actions/runs/34973438029) proves WebKit sandbox read/extension denials after accepted selection; the host verifies the first 529,864-byte file digest. Isolated one-file and full-batch inputs report `NotReadableError` for `File.arrayBuffer()`, bounded slice and `FileReader`, plus `TypeError` for the blob stream. Chromium/Firefox pass the same run's real 94-file study and pixel oracles.
...
- Status: in-progress; priority: P3; owner: Metis integration; integrator: root; last-update: 2026-09-15; branch: `docs/metis-state-reconciliation`; regions: `backlog.md`; risk: structural conformance debt
