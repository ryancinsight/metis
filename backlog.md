...
- Current increment (2026-09-11, branch `feat/metis-attribute-trace`): add a generic bounded DOM-attribute snapshot to the canvas trace so consumers can supply their own semantic attribute names. Metis records values and enforces size/count limits without interpreting DICOM or viewer meaning; RITK will consume the generic evidence through its manual workflow.
