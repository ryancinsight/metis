...
- Completed increment (2026-09-14, hosted RITK workflow [34858003647](https://github.com/ryancinsight/ritk/actions/runs/34858003647)): Chromium and Firefox accepted the real 94-file MRI-DIR T2 study, matched all file and three-canvas RGBA oracles, rejected the bounded overflow probes and closed cleanly. Safari 26.6.2 accepted the chooser paths but rejected the first bounded `Blob.arrayBuffer()` read; the exact galleries and provenance are recorded in [RITK's manual evidence](https://github.com/ryancinsight/ritk/tree/main/docs/manual/images). Metis `e3d1cdb`, RITK `6fabf70` and Moirai `3746941` are the source revisions.
...
- Current increment: `metis-platform::native::WebViewSurface` consumes the merged provider through a safe adapter. It sizes the controller, maps validated visibility, forwards combined window/WebView events and preserves bounded navigation and JSON messaging. Metis uses git-plus-version Moirai requirements; Cargo.lock records provider revision `3746941f810d696ced5bd7de6f1eb391e2857207`. Configuration and ignored installed-runtime adapter tests pass on WebView2 `152.0.4191.66`, covering packaged navigation, the ready bridge message, external-navigation denial and close. The visible bundle journey is captured in [`docs/manual/images/native-captures.json`](docs/manual/images/native-captures.json).
...
- Current increment (2026-09-14): Metis PR [#145](https://github.com/ryancinsight/metis/pull/145), merge `02d4047c5567834667ab9beb796ea27f6257f0ad`, adds the `after-keyboard` semantic canvas snapshot after each focused `ArrowDown` pair. The complete local gate at Metis `d69c70732341dba59a44303955851fa3cdc9b037` passes all stages (170 resolved packages); the dependency-free browser suite passes 60/60. [RITK PR #382](https://github.com/ryancinsight/ritk/pull/382) consumes this boundary and its hosted keyboard validation remains pending.
