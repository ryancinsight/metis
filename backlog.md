...
- Status: in-progress; priority: P1; owner: Metis frontend/host; integrator: root; last-update: 2026-09-14; branch: `fix/metis-small-file-read`; regions: crates/metis-web/src/browser/file_drop.rs, backlog.md; dependencies: METIS-STATE-001, METIS-ASYNC-001, METIS-AUTHORITY-001; risk: browser/native trust boundary
...
- Active provider-read increment (2026-09-15, Metis branch `fix/metis-small-file-read`, Moirai PR [#344](https://github.com/ryancinsight/Moirai/pull/344)): the consumer sizes its first bounded request to the file when it is at most 1 MiB, allowing Moirai's `File.arrayBuffer()` path for Safari while retaining object-URL continuation reads for larger or positioned requests. Local WASM, Clippy, nextest 40/40 and Python browser tests pass; hosted RITK replay remains the acceptance oracle.
