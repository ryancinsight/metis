...
- Current increment: the format-neutral canvas runner can focus each canvas and record a trusted `ArrowDown` keydown/keyup pair with bounded key, code, repeat and modifier metadata; the local dependency-free suite passes 47/47. The manual now documents the keyboard trace command and RITK validation boundary. Hosted RITK validation remains pending.
- Completed increment (2026-09-14): Metis PR [#143](https://github.com/ryancinsight/metis/pull/143), merge `0c3444d1b37b8d16813fa45d825b91027b1b5b1b`, delivers the format-neutral keyboard trace runner and its standard chooser path; the dependency-free browser suites pass 60/60 (`test_browser*.py`). RITK hosted validation is running in workflow dispatch `34889847579` against RITK main `7f5af11d193b8c31c7db9efa704e34089a4930fb` with this exact Metis revision.
...
- Browser keyboard snapshot increment (2026-09-14): Metis PR [#145](https://github.com/ryancinsight/metis/pull/145), commit `0ca889f`, records an `after-keyboard` canvas snapshot after each focused `ArrowDown` pair. The RITK consumer can therefore validate wheel progression from the state immediately before the wheel instead of comparing a keyboard-plus-wheel final state with the initial state.
