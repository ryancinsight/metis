...
- Completed increment (2026-09-18): `--accessibility-probe` now queries Chromium and Microsoft Edge's bounded `Accessibility.getFullAXTree` endpoint at depth eight and records role/count/name-presence evidence in `metrics.accessibility_native_tree`; Firefox and WebKit emit an explicit protocol-unavailable record. Focused accessibility tests cover available, unavailable and malformed trees. This keeps the required workbench roles/names within the native response bound without claiming spoken output or OS bridge support.
