...
- Completed increment (2026-09-11, Metis `78221299df10222017cd1077b2288c28ff5abada`, RITK `0bad9eb2c77b91e57fff3e9dc472a42201e061aa`): the paired canvas capture runs before overflow probes, so its initial and final window PNGs show the accepted study while rejection checks compare against the post-input RGBA baseline. Edge records trusted pointer and wheel events for all three canvases; RITK validates the six snapshots, six actions, eight PNGs and released 12 diagnostic listeners. The trace and images are in [`docs/manual/images`](docs/manual/images/browser-gallery-canvas.json).
