...
- Completed increment (2026-09-17): `scripts/browser_input_latency.py` measures repeated trusted WebDriver clicks from a stable selector to the next `requestAnimationFrame`, validates timestamp and target identity, records mean/population spread/minimum/maximum and releases its listener. Focused contract tests cover positive, negative, bound and closed-session output paths; the manual and V12 record the browser-boundary limits.
