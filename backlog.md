...
- Completed increment (2026-09-18): `--accessibility-probe` now queries Chromium and Microsoft Edge's bounded `Accessibility.getFullAXTree` endpoint at depth eight, byte-bounds the response while recording role/count/name-presence evidence in `metrics.accessibility_native_tree`; Firefox and WebKit emit an explicit protocol-unavailable record. Focused accessibility tests cover available, unavailable and malformed trees. This retains the required workbench roles/names without claiming spoken output or OS bridge support.
