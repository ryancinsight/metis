...
- Documentation increment (2026-09-11): the application manual now links the RITK-reviewed native capture and all three actual browser-canvas frames produced from the public MRI-DIR CT series. The images are runtime output through the format-neutral Métis framebuffer/canvas; DICOM parsing, geometry, and clinical display remain RITK-owned, and private studies remain local. Provenance records the 409-file, 216,156,416-byte bounded browser handoff and frame hashes; physical drag-and-drop and cross-engine driver evidence remain open.
