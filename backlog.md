...
- Status: in-progress; priority: P1; integrator: root; last-update: 2026-09-16; delivery: [PR #203](https://github.com/ryancinsight/metis/pull/203); decision: [ADR 0025](docs/adr/0025-axum-server-boundary.md).
...
- Replay: configured local gate passes at `c1ee7db` (source digest `a9f7091abd39b56d896b98fcb71d90d3c882a77bc60e9c07ceabb542c3ae46d0`); hosted `35173142639` passes all three browser jobs, all nine PNG hashes and trace semantics verify, and WebKit's three PNGs are inspected. Its Windows gate instead times out a Node file-drop fault harness before native tests; hosted cause is unproven. Review rejects the microtask replacement because a missing cleanup await escapes it; the fault now rejects on the next event-loop turn, retaining the ten-second process bound and all cleanup assertions. The missing-await mutation fails, and 32 focused tests pass.
- Remaining defect: final local native run at `ff36301` fails idle-peer recovery in 2.320 seconds with observed peer failures `[Timeout]` (329 tests pass). The external observer reproduces the outstanding HTTP stall despite the PAL cleanup repair. [PR #204](https://github.com/ryancinsight/metis/pull/204) stays draft; phase run `af00da25` fails in 2.263 seconds after connect/write and server completion, with no client read completion. Forwarding-waker diagnostic runs `3ccd20c7` (debug), `1f39e345` (release), and the stable-waker run (idle recovery 0.402 seconds) pass all 330 tests; they do not negate the reproduced failure. Hosted gate `35173910225` passes at merge revision `b7e8244`; its 305 normalized source fingerprints match `ff36301`.
