...
- Follow-on provider: Moirai branch `arch/webview2-provider` at `0310280a` adds the thread-affine WebView2 host, packaged `file:///` navigation policy, bounded WebMessageReceived bridge and callback teardown. Its installed-runtime smoke passes on WebView2 `152.0.4191.66`, and the visible Metis bundle journey is captured in the linked manifest; the [Moirai provider item](../moirai/docs/backlog.md#MOI-WINDOW-WEBVIEW2-2026-09-09) remains in progress only through branch integration. No registry or signing key is part of that provider.
