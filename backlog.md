...
- Completed increment (2026-09-11, local branch `feat/metis-browser-canvas-001`): `metis-web` exposes a format-neutral borrowed `CanvasFrame` seam and a WASM `CanvasSurface` over Moirai's bounded HTML5 canvas provider. RITK's `PresentationFrame` is the consumer adapter; no DICOM parser, metadata, geometry or viewer state enters Métis. Native tests and strict native/WASM clippy pass; a live browser canvas capture and cross-engine driver evidence remain open.
