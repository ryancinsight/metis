...
- Status: in-progress; priority: P1; integrator: root; last-update: 2026-09-17; delivery: [PR #203](https://github.com/ryancinsight/metis/pull/203); follow-up: [PR #204](https://github.com/ryancinsight/metis/pull/204) (draft); branch: `fix/metis-idle-peer-deadline`; decision: [ADR 0025](docs/adr/0025-axum-server-boundary.md).
...
- Metis delivery: `743390e` merges bounded per-peer failure reporting with two intentional public observer-argument additions and migrated callers/docs. `ff36301` retains the ten-second Node fault-harness bound while making missing cleanup await fail. `00e41aa` bounds entire verification process trees; 32 focused tests and the 213-test Python suite pass (one existing skip). POSIX runtime coverage awaits the Linux browser-assets check.
- Provider delivery: [Moirai #386](https://github.com/ryancinsight/Moirai/pull/386) fixes reproduced `POLLNVAL` waiter retention; [#387](https://github.com/ryancinsight/Moirai/pull/387) retains terminal driver errors and wakes/rejects affected operations; [#388](https://github.com/ryancinsight/Moirai/pull/388) repairs the stale source-contract test. The consumer's eleven Moirai source pins advance to `a3a0ae14`; upstream configured verification passes 1,117 tests and its hosted gate. Older Metis stall causality remains unproven.
- Reproduced stall: local `ff36301` fails idle recovery after 2.320 seconds with peer failures `[Timeout]`; phase run `af00da25` reaches server completion but not client read completion. Passing diagnostic runs and hosted run `35173910225` do not negate that failure. Diagnostic wrappers are removed; the committed observer cannot repoll the request.
- Current measured failure: release run `78c768c9` at provider `a3a0ae14` fails idle recovery in 0.422 seconds with retained Winsock error `10038` (not a socket); debug passes 330/330 and release passes 329/330, one skip each. Evidence: `output/fragment-driver-socket-error/{verification.json,tests.log,release-tests.log}`. Socket close overlapping polling is the owning hypothesis, not an established historical cause.
...
- Dependency: [owning Windows socket-lifetime work](../moirai/docs/backlog.md#MOI-WINDOWS-SOCKET-LIFETIME-2026-09-17); final consumer adoption and browser replay follow its measured result. Close only after the acceptance oracle passes on the final revision.
