...
- Lease: root — `crates/metis-backend/src/supervisor.rs`, `crates/metis-app/src/backend.rs`, native capture evidence and manual records — 2026-09-09T12:37:00-04:00
...
- Follow-on provider: Moirai branch `arch/webview2-provider` at `0310280a` adds the thread-affine WebView2 host, packaged `file:///` navigation policy, bounded WebMessageReceived bridge and callback teardown. Its installed-runtime smoke passes on WebView2 `152.0.4191.66`; the [Moirai provider item](../moirai/docs/backlog.md#MOI-WINDOW-WEBVIEW2-2026-09-09) remains in progress until the visible Metis bundle journey is captured. No registry or signing key is part of that provider.
- Current increment: `metis-platform::native::WebViewSurface` now consumes that provider through a safe adapter. It sizes the controller, maps validated visibility, forwards combined window/WebView events and preserves bounded navigation and JSON messaging. All direct Moirai dependencies are pinned to the same provider commit for co-evolution; the pin is removed after the provider merges to Moirai main. Configuration and ignored installed-runtime adapter tests pass on WebView2 `152.0.4191.66`, covering packaged navigation, the ready bridge message, external-navigation denial and close; the visible bundle capture remains open.
- Current increment: `metis-app --metis-webview` now runs the same one-executable supervised process workflow through a visible WebView2 form. A bounded temporary HTML/CSS package uses a no-network CSP and typed JSON bridge; submit messages traverse the unprivileged frontend's private pipe and return the capability-authorized, audited result to the page. The application installed-runtime form journey, visible capture, permission and accessibility evidence remain open.
