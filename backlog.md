...
- Current increment (2026-09-12): the bounded capture utility decodes every PNG scanline filter and rejects malformed checksums before presenting the exact RGBA pixels from the public RITK 409-file CT/MIP frame. The visible 1296 × 839 window shows four non-black RITK panels; source and output digests are recorded in [the capture manifest](docs/manual/images/python-native-captures.json). Moirai provider rendering is tracked by [PR #329](https://github.com/ryancinsight/Moirai/pull/329). Private patient frames remain local; DICOM ownership remains RITK.
