...
- Current evidence: Moirai PR #283 (`3ae43143`) now supplies the real Win32 HWND, bounded message translation, retained ARGB presentation and finite queue waiting. `metis-platform::native::NativeSurface` is the safe consumer boundary; `PlatformSurface` and `PlatformEvent` remain portable application-supplied values.
