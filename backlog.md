...
- Evidence: dependency-free checks pass 281/281 with one intentional skip; the pinned full Metis verifier passes through doctests, with the expected `capture-failure` probe returning `PermissionDenied` and zero visual pixel or semantic differences. The hosted cross-engine comparator is configured for scheduled/manual runs and remains pending until that workflow executes.
...
- Completed increment (2026-09-18): `--accessibility-probe` now queries Chromium and Microsoft Edge's bounded `Accessibility.getFullAXTree` endpoint at depth eight, byte-bounds the response and caps visible nodes while recording role/count/name-presence evidence in `metrics.accessibility_native_tree`; Firefox and WebKit emit an explicit protocol-unavailable record. Focused accessibility tests cover available, unavailable and malformed trees. This keeps the required workbench roles/names within the semantic bound without claiming spoken output or OS bridge support.
