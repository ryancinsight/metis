...
- Status: in-progress; priority: P1; owner: Metis measurement; integrator: root; last-update: 2026-09-13; branch: `feat/browser-heap-metric-metis-perf-001`; regions: `scripts/browser_trace.py`, `scripts/browser_canvas.py`, `scripts/browser_runtime.py`, `scripts/tests/test_browser_runtime.py`, `docs/manual/browser.md`, `docs/manual/testing.md`, `docs/VERIFICATION.md`; dependencies: METIS-BROWSER-001, METIS-DESKTOP-001, METIS-INPUT-001; risk: invalid comparative claims
...
- Current increment: add an optional browser JavaScript-heap sample to the format-neutral canvas trace. The sample is recorded only when the engine exposes `performance.memory`, bounded and validated as browser heap evidence; it does not claim WASM, native process, compositor or allocation-profiler measurements.
